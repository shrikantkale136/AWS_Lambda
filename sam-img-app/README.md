...
```bash
sam-img-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-img-app$ sam local start-api
sam-img-app$ curl http://localhost:3000/
```
...
```bash
sam-img-app$ sam logs -n HelloWorldFunction --stack-name sam-img-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-img-app
```
